...
$('#product-size-selector').change(function () {
  ...
  var ActionRoute = $(this).data('action'); //Send API Request to get available colors
  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      $('#choose-product-color').html(''); //Plug the colors

      response.forEach(function (color, i) {
        $('#choose-product-color').append("\n              <div class=\"sc-item\">\n                  <input type=\"radio\" name=\"color\" id=\"color-".concat(i, "\" value=\"").concat(color.color_code, "\">\n                  <label for=\"color-").concat(i, "\" style=\"background:").concat(color.color_code, ";\"></label>\n              </div>\n            "));
      });
    },
